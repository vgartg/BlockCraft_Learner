...
class GameEngine:
    ...
    def generate_world(self):
        ...
        try:
            ...
            self.world = [['air' for _ in range(world_width)] for _ in range(world_height)]
                ...
                if x % 10 == 0 and x > 10 and x < world_width - 10:
                    ...
                    for dy in range(-2, 1):
                        for dx in range(-2, 3):
                            ...
                            lx = tree_x + dx
                            if 0 <= lx < world_width and 0 <= ly < world_height and abs(dx) + abs(dy) < 4:
            ...
            self.world = [
                [
                    'grass' if y == 10 else 'air'  # Создаем простой мир при ошибке
                    for x in range(50)
                ]
                for y in range(20)
            ]
    ...
    def get_block_at(self, x, y):
        ...
        try:
            ...
            if 0 <= grid_y < len(self.world) and 0 <= grid_x < len(self.world[0]):
    ...
    def place_block(self, x, y, block_type):
        ...
        try:
            ...
            if grid_x >= 0 and grid_y >= 0 and block_type in self.inventory and self.inventory[block_type] > 0:
                ...
                player_rect = pygame.Rect(
                    self.player['x'], self.player['y'], self.config['BLOCK_SIZE'], self.config['BLOCK_SIZE'] * 2
                )
    ...
    def break_block(self, x, y):
        ...
        try:
            ...
            if grid_x >= 0 and grid_y >= 0 and block_type != 'air' and block_info.get('breakable'):
    ...
    def check_collision(self, x, y, width, height):
        ...
        for block_y in range(top, bottom + 1):
            for block_x in range(left, right + 1):
                if 0 <= block_y < len(self.world) and 0 <= block_x < len(self.world[0]):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key in [pygame.K_1, pygame.K_KP1]:
                    ...
                    custom_blocks = [
                        b for b in self.blocks.keys() if b not in ['air', 'grass', 'dirt', 'stone', 'wood', 'leaf']
                    ]
                    if custom_blocks:
    ...
    def update_player(self):
        ...
        # Ограничение камеры
        self.camera_x = max(0, min(self.camera_x, world_width - self.config['SCREEN_WIDTH']))

        world_height = self.config['WORLD_HEIGHT'] * self.config['BLOCK_SIZE']
        self.camera_y = max(0, min(self.camera_y, world_height - self.config['SCREEN_HEIGHT']))
    ...
    def run(self):
        ...
        while running:
            ...
            self.renderer.draw_hud(self.player, self.selected_block, self.inventory, fps, self.blocks)
