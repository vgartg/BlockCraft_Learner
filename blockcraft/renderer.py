...
class Renderer:
    ...
    def draw_block(self, x, y, block_type):
        ...
        if block_type in self.block_textures:
            ...
            pygame.draw.rect(self.screen, color, (x_int, y_int, self.config['BLOCK_SIZE'], self.config['BLOCK_SIZE']))
            pygame.draw.rect(
                self.screen,
    ...
    def draw_world(self, world, camera_x, camera_y):
        ...
        for y in range(start_y, end_y):
            for x in range(start_x, end_x):
                ...
                if -block_size <= screen_x_int <= screen_width and -block_size <= screen_y_int <= screen_height:
    ...
    def draw_player(self, player, camera_x, camera_y):
        ...
        pygame.draw.rect(
            self.screen,
            player['color'],  # Тело игрока
            (screen_x, screen_y, self.config['BLOCK_SIZE'], self.config['BLOCK_SIZE'] * 2),
        )

        # Глаза (Белым блоком)
        eye_size = self.config['BLOCK_SIZE'] // 4
        pygame.draw.rect(self.screen, (255, 255, 255), (screen_x + eye_size, screen_y + eye_size, eye_size, eye_size))
        pygame.draw.rect(
            self.screen,
            (255, 255, 255),
            (screen_x + self.config['BLOCK_SIZE'] - eye_size * 2, screen_y + eye_size, eye_size, eye_size),
        )
    ...
    def draw_hud(self, player, selected_block, inventory, fps, blocks):
        ...
        pygame.draw.rect(
            self.screen,
            (50, 50, 50, 200),
            (0, self.config['SCREEN_HEIGHT'] - panel_height, self.config['SCREEN_WIDTH'], panel_height),
        )
        ...
        color = (
            (255, 255, 255) if inventory.get(selected_block, 0) > 0 else (255, 0, 0)
        )  # В зависимости от количества выбранного блока
        pygame.draw.rect(self.screen, color, (selected_x - 2, selected_y - 2, 44, 44), 2)
        ...
        draw_text(self.screen, f'Выбран: {block_name}', (selected_x + 50, selected_y + 10), 24, (255, 255, 255))

        # Координаты игрока
        draw_text(self.screen, f'X: {int(player["x"])} Y: {int(player["y"])}', (10, 10), 20, (255, 255, 255))

        # FPS
        draw_text(self.screen, f'FPS: {fps}', (self.config['SCREEN_WIDTH'] - 100, 10), 20, (255, 255, 255))
        ...
        for i, (block_type, count) in enumerate(inventory.items()):
            ...
            if selected_block == block_type:
                pygame.draw.rect(self.screen, block_color, (inv_x + i * 60 - 2, selected_y - 2, 44, 44), 2)
            self.draw_block(inv_x + i * 60, selected_y, block_type)
            draw_text(self.screen, str(count), (inv_x + i * 60 + 30, selected_y + 30), 16, text_color)
